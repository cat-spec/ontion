...
const translateServiceBaseUrl = process.env.NEXSCRM_TRANSLATE_BASE_URL || 'http://localhost:8000'
...
async function requestTranslateService<T>(pathname: string, init?: RequestInit) {
...
function normalizeLanguageResponse(payload: unknown) {
...
function normalizeTranslateResponse(payload: unknown) {
  if (!payload || typeof payload !== 'object') {
    return null
  }

  const response = payload as { code?: number; data?: unknown }
...
ipcMain.handle(platformIpcChannels.languageList, async () => {
  try {
    const payload = await requestTranslateService<unknown>('/api/languageList/languageList')
...
ipcMain.handle(platformIpcChannels.translateText, async (_event, args: {
  text: string
  local: string
  target: string
}) => {
  try {
    const payload = await requestTranslateService<unknown>('/api/translate', {
      method: 'POST',
      body: JSON.stringify({
        fromLang: args.local,

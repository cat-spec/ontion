...
describe('client store navigation', () => {
  ...
  it('creates a session for the currently active platform by default', async () => {
    const { useClientStore } = await import('@/stores/client')
    const store = useClientStore()

    bridgeMock.createSession.mockImplementation(async (session) => ({
    ...
    store.setActivePlatform('whatsapp')
    await store.createSession()

    expect(bridgeMock.createSession).toHaveBeenCalledWith(expect.objectContaining({
      id: 'whatsapp-0001',
      platformId: 'whatsapp',
